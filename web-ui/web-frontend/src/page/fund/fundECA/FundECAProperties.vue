...
<script>
   ...
   import FundEcaAllot from './FundECAallot.vue'
   export default{
        components: {
            FundEcaAllot
        }
        ...
        methods:{
            ...
            getFundECAInfo: function(){
                var _self = this;
                _self.winsigns.ds.GET({url:_self.winsigns.api.fund.getECAByIdURL(_self.$route.params.fundECAId),
                        data:{}},function(data){                    
                    _self.fundCreaAccontInfoList = data;  
                    _self.fundCreaAccontInfoCapList = [];
                    if ( data._embedded){
                        if (data._embedded["eca-cash-pools"]){
                            _self.fundCreaAccontInfoCapList = data._embedded["eca-cash-pools"];
            ...
            moneySubscribe: function(message){
                ...
                for (var i=0;i<=_self.fundCreaAccontInfoCapList.length-1;i++){
                    if ('/topic/'+_self.fundCreaAccontInfoCapList[i].measureInfo["ECACashPoolMHT.ECACashMeasure"].key
                        ==message.headers.destination){
                        _self.fundCreaAccontInfoCapList[i]
                        this.$set(_self.fundCreaAccontInfoCapList[i],"ecma",message.body);
                    };
                }
            }
        },
